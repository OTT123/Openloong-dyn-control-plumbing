...
public:
  ...
  const std::vector<std::string> motorName = {
      ...
      "RR_hip_joint",
      "RR_thigh_joint",
      "RR_calf_joint"}; // joint name in urdf and jason config files
  ...
  pinocchio::JointIndex FL_calf_joint_, FR_calf_joint_, RL_calf_joint_,
      RR_calf_joint_;
  ...
  pinocchio::FrameIndex FL_foot_frame_, FR_foot_frame_, RL_foot_frame_,
      RR_foot_frame_;
  pinocchio::FrameIndex FL_foot_frame_fixed_, FR_foot_frame_fixed_,
      RL_foot_frame_fixed_, RR_foot_frame_fixed_;
  ...
  Eigen::Vector3d FL_foot_pos_W_, FR_foot_pos_W_, RL_foot_pos_W_,
      RR_foot_pos_W_; // foot-end position in world frame
  Eigen::Vector3d FL_foot_pos_L_, FR_foot_pos_L_, RL_foot_pos_L_,
      RR_foot_pos_L_; // foot-end position in body frame
  Eigen::Vector3d FL_foot_vel_L_, FR_foot_vel_L_, RL_foot_vel_L_,
      RR_foot_vel_L_; // foot-end velcity in body frame
  ...
  Eigen::Matrix3d FL_thigh_rot_W_, FR_thigh_rot_W_, RL_thigh_rot_W_,
      RR_thigh_rot_W_;
  Eigen::Matrix3d FL_foot_rot_L_, FR_foot_rot_L_, RL_foot_rot_L_,
      RR_foot_rot_L_; // in Body frame (base)

  Eigen::Matrix3d base_rot_; // in world frame
  Eigen::Vector3d base_pos_; // in world frame

  enum legIdx { FL, FR, RL, RR };
  struct IkRes {
    int status;
    int itr;
    Eigen::VectorXd err;
  ...
  void computeDyn();
  IkRes
  computeInK_Leg(const Eigen::Matrix3d &Rdes_FL, const Eigen::Vector3d &Pdes_FL,
                 const Eigen::Matrix3d &Rdes_FR, const Eigen::Vector3d &Pdes_FR,
                 const Eigen::Matrix3d &Rdes_RL, const Eigen::Vector3d &Pdes_RL,
                 const Eigen::Matrix3d &Rdes_RR,
                 const Eigen::Vector3d &Pdes_RR);
  IkRes computeInK_Leg_pos_only(const Eigen::Vector3d &Pdes_FL,
                                const Eigen::Vector3d &Pdes_FR,
                                const Eigen::Vector3d &Pdes_RL,
                                const Eigen::Vector3d &Pdes_RR);
  IkRes computeInK_SingleLeg(const Eigen::Matrix3d &Rdes,
                             const Eigen::Vector3d &Pdes,
                             const legIdx &leg_idx);
